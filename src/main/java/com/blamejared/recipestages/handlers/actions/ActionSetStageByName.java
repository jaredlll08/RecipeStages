...
public class ActionSetStageByName extends ActionStageBase {
    ...
    public String describe() {
        return "Setting the stage of  \"" + Registry.RECIPE_TYPE.getKey(this.getManager().getRecipeType()) + "\" recipes with name: " + this.name + "\" to \"" + stage + "\"";
    }
    
    public boolean validate(ILogger logger) {
        boolean containsKey = this.getManager().getRecipes().containsKey(this.name) || this.getManager().getRecipes().containsKey(new ResourceLocation("recipestages", this.name.toString().replaceAll(":", "_")));
        if(!containsKey) {
            logger.warning("No recipe with type: \"" + Registry.RECIPE_TYPE.getKey(this.getManager().getRecipeType()) + "\" and name: \"" + this.name + "\"");
        }
